...
# serving the GET method of "users", the "cursor.fetchone" returns a tuple
...
# serving the POST method of "users", checks that the id is a number and not already taken
# and then, send the request to the db. if all goes well, returns 1 else returns 0
def db_post(user_id, user_name, creation_time):
    ...
    if cursor.rowcount > 0:
        print("id already exists.")  # for debugging
        return 0
    try:
        cursor.execute(
...
# serving the PUT method of "users", checks that the id exists. if it does.
